...
```
Every night at 2 AM UTC, run !generate-testdata against Issues labeled testdata-refresh in the codev-workshops/otterworks repo, namespace nightly.
```
...
```
When a GitHub Issue is labeled testdata on the codev-workshops/otterworks repo, start a Devin session that invokes !generate-testdata with the issue number and namespace derived from the issue ID.
```
